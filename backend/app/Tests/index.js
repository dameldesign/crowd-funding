import * as user from "./players_profile.js";
...
//{"method":"create_profile","name":"0xreadyPlayer1","logoURI":"X7sdsa8ycn"}
function createProfile(name, owner, logoURI){
  console.log("creating profile....");
  const createdProfile = user.createUser( name,  owner, logoURI);
...
function getUser(owner){
  let user = user.getUser(owner);
  console.log("getting  profile....");
  console.log("player profile: " + JSON.stringify(user));
}
...
function createTeam(owner, char1, char2, char3){
  ...
  let player = user.findUser(user.allUsers, owner);
...
function deposit(userAddress, amount) {
  ...
  let userB4Deposit = user.getUser(userAddress);
  console.log("user initial profile:", JSON.stringify(userB4Deposit));
  let player = marketplace.deposit(userAddress, amount);
  let userAfterDeposit = user.getUser(userAddress);
  console.log("user final profile:", JSON.stringify(userAfterDeposit));
}


function withdraw(userAddress, amount) {
  console.log("withdrawing...")
  let userB4Withdraw = user.getUser(userAddress);
  let player = marketplace.withdraw(userAddress, amount);
  let userAfterWithdraw = user.getUser(userAddress);
  // if (userAfterWithdraw != (userB4Withdraw - amount)) {
  //   throw new Error("Withdraw test failed");
  // }
  console.log(JSON.stringify(userAfterWithdraw));
}

function transferToken(userAddress, receiverAddress, amount) {
  console.log("transferring...")
  let userB4Transfer = user.getUser(userAddress);
  let receiverProfileB4Transfer = user.getUser(receiverAddress);
  let transfer = marketplace.transferToken(userAddress, receiverAddress, amount);
  let userAfterTransfer = user.getUser(userAddress);
  let receiverProfileAfterTransfer = user.getUser(receiverAddress);
  // if (userAfterTransfer != (userB4Transfer - amount) && receiverProfileAfterTransfer != receiverProfileB4Transfer + amount) {
  //   throw new Error("Transfer test failed");
  // }
  console.log(JSON.stringify(userAfterTransfer));
}
...
function BuyCharacter(characterId, buyerAddress) {
  ...
  console.log("sellers's Profile: " + JSON.stringify(user.getUser("0x12896191de42EF8388f2892Ab76b9a728189260A")))
...
getUser("0x12896191de42EF8388f2892Ab76b9a728189260A")

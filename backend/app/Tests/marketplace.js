// const { allUsers, findUser } = require("./players_profile.js");
import { allUsers, findUser } from "./players_profile.js";
...
function deposit(userAddress, amount) {
   let player = findUser(allUsers, userAddress);
...
function transferToken(userAddress, receiverAddress, amount) {
    if(!findUser(allUsers, receiverAddress)) {
        throw new Error("Receiver Not a registered player");
    }
    let player = findUser(allUsers, userAddress);
    let receiver = findUser(allUsers, receiverAddress);
    let playerNebulaBalance = player.NebulaTokenBalance;
    if (playerNebulaBalance < amount) {
     throw new Error("Insufficient balance")
    }
...
function BuyCharacter(characterId, buyerAddress) {
  console.log("debug:", listedCharacters);
    ...
    if(!findUser(allUsers, buyerAddress)) {
        throw new Error("Buyer Not a registered player");
    }
    let buyer = findUser(allUsers, buyerAddress);
    ...
    let seller = findUser(allUsers, listedCharacters[characterIndex].seller);
